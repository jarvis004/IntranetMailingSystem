...
<li style = "background-color:#9fd7fb; width: 230px; height:30px; padding-top: 5px; font-size: 18px;"><a href="sent.php"  style="color:#326ada; "> &emsp; Sent Mails</a></li>
...
<?php
	if (isset($_COOKIE['ies'])){
		...
		$prev=$page-1;
		$next=$page+1;
		echo "<div class=\"nav_links\"><a href=sent.php?page=".$prev." style=\"float:left;\">previous</a>";
		print "<span>Showing:  ".(($page+1)*10-9)." to ".(($page+1)*10)."</span>";
		echo "<a href=sent.php?page=".$next." style=\"float:right;\">next</a></div>";
		$leave=$page*10;
	
		$query1="select msg_id from  mailstats where username='$user' and type='snt' order by serial DESC limit ".$leave.",10";
		...
		if(mysql_num_rows($result1)){
			...
			/*foreach($_POST as $del_msgs){
					if(is_array($del_msgs)){
						foreach($del_msgs as $i){
							echo $i;
						}
					}
					else{
...
/*	if (isset($_COOKIE['ies'])){
		...
		$prev=$page-1;
		$next=$page+1;
		echo "<a href=sent.php?page=".$prev.">previous</a>";
		...
		if(mysql_num_rows($result1)){
			...
			while($rowset1=mysql_fetch_array($result1)){
				...
				echo "<tr>";
				echo "<td>".($leave + ++$i)."</td>";
				echo "<td ><input type=\"checkbox\" name=\"del_msgs[]\" value=".$id."><a class=\"msglink\" href=\"showmail.php?id=".$id."\">".$rwst2['subject']."</a></td>";
				echo "</tr>";
			}
			echo "</table>";
